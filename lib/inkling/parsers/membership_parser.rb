  ...
  module Parsers
    class MembershipParser < BaseParser
      ...
      def parse_membership_node(membership_node)
        ...
        membership.type = attrs[:name]
        attrs[:kids].each do |node|
          if node[:name] == "user"
            node[:kids].each do |unode|
              attr_name = unode[:name].gsub("-", "_")
              membership.send("#{attr_name}=", parse_node_value(unode)) if membership.respond_to?("#{attr_name}=")
            end
          else
            attr_name = node[:name].gsub("-", "_")
            membership.send("#{attr_name}=", parse_node_value(node)) if market.respond_to?("#{attr_name}=")
          end
        end
